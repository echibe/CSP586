...
document.head.appendChild(import1);    //head is parent and chart.bundle is child
...
class stack_chart extends bar_chart{
    ...
    var Combined = new Array();
	Combined[0] = ['First', 'Second','Third'];
	for (var i = 0; i < First.length; i++){
		console.log(First[i]);
		console.log(Second[i]);
		Combined[i + 1] = [First[i], Second[i] ,Third[i]];
		}
		//second parameter is false because first row is headers, not data.
	var table = google.visualization.arrayToDataTable(Combined, false);
				 
	var options = {
    title: 'Using Google Chart' , isStacked:true
	
        };
		
	var chart = new google.visualization.BarChart(document.getElementById('stackchart'));
